  ...
  constructor ({req} = {}) {
    ...
    try {
      if (req) {
        ...
        this._session = {
          csrfToken: req.session.__attributes._token
        }
        // If the session is associated with a user add user object to session
        if (req.session && req.session.__attributes.data) {
          const data = req.session.__attributes.data
          this._session.user = data.user
          this._session.expires = data.expires
        }
